...
interface ChatMessageProps {
  ...
  from: "me" | "them";
  time?: string;
  status?: "sent" | "read"; // hanya untuk "me"
}
...
export default function ChatMessage({ message, from, time, status }: ChatMessageProps) {
  ...
  return (
    <div className={`flex items-end space-x-3 ${isMe ? "justify-end" : "justify-start"}`}>
      ...
      {/* {!isMe && (
        <div className="w-9 h-9 rounded-full bg-blue-700 text-white flex items-center justify-center font-semibold text-lg select-none">
          T
        </div>
      )} */}
      ...
      <div className="max-w-[70%] flex flex-col">
        <div
          className={`px-5 py-3 rounded-2xl shadow-sm text-sm leading-snug break-words ${
            isMe
